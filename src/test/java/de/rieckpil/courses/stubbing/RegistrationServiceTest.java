...
public class RegistrationServiceTest {
  ...
  void basicStubbingWithArgumentMatchers() {
    when(bannedUsersClient.isBanned(eq("duke"), ArgumentMatchers.any(Address.class))).thenReturn(true);

    when(bannedUsersClient.isBanned(ArgumentMatchers.anyString(), ArgumentMatchers.isNull())).thenReturn(true);

    when(bannedUsersClient.isBanned(ArgumentMatchers.argThat(s -> s.length() <= 3), ArgumentMatchers.isNull())).thenReturn(false);

    System.out.println(bannedUsersClient.isBanned("duke", new Address()));
    System.out.println(bannedUsersClient.isBanned("hagjfg", null));
  ...
  void basicStubbingUsageThenAnswer() {
    ...
    System.out.println(bannedUsersClient.isBanned("duke", berlin));

    Address london = new Address();
    london.setCity("London");
    System.out.println(bannedUsersClient.isBanned("duke", london));
  ...
  void shouldNotAllowRegistrationOfBannedUsers() {
    ...
    assertThrows(IllegalArgumentException.class, () -> cut.registerUser("duke", Utils.createContactInformation("duke@test.com")));
  }
  ...
  void shouldAllowRegistrationOfNewUser() {
    ...
    User user = cut.registerUser("duke", Utils.createContactInformation("duke@test.com"));
